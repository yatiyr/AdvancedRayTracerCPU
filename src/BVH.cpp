...
bool BVH::Intersect(const Ray& ray, IntersectionReport& report, float tmin, float tmax, float intersectionTestEpslion, bool softShadingFlag)
{
    ...
    if(this->leftChild == nullptr && this->rightChild == nullptr)
    {
        for(size_t i=0; i<primitives.size(); i++)
        {
            ...
            bool hitTest = primitives[i].Intersect(ray, r, tmin, tmax, intersectionTestEpslion, softShadingFlag);
            if(hitTest)
            {
    ...
    bool leftTest  = leftChild ->Intersect(ray, report1, tmin, tmax, intersectionTestEpslion, softShadingFlag);
    bool rightTest = rightChild->Intersect(ray, report2, tmin, tmax, intersectionTestEpslion, softShadingFlag);
